...
#define SPTWB_DATA_LENGTH 512 // 128KB
#define SECTOR_SIZE_SHIFT_BIT 9
...
int iReadViaReadFileEx(HANDLE _hDevice, PVOID databuffer, ULONG SectorCnt, ULONG lba) {
    ...
    overlapRead.Pointer = 0;
    overlapRead.hEvent = 0;

    overlapRead.Pointer = (PVOID)(lba << 9);

    BOOL result = ReadFileEx(_hDevice, databuffer, (SectorCnt << 9), &overlapRead, ReadFileIOCompletionRoutine);
...
int iWriteViaWriteFileEx(HANDLE _hDevice, PVOID databuffer, ULONG SectorCnt, ULONG lba) {
    ...
    overlapWrite.Pointer = 0;
    overlapWrite.hEvent = 0;

    overlapWrite.Pointer = (PVOID)(lba << 9);
    BOOL result = WriteFileEx(_hDevice, databuffer, (SectorCnt << 9), &overlapWrite, WriteFileIOCompletionRoutine);
...
int iWriteViaSCSIPassThroughEx(HANDLE _hDevice, PVOID databuffer, ULONG SectorCnt, ULONG lba, ULONG timeoutVal) {
    ...
    sptdwb.sptd.DataTransferLength = SectorCnt << SECTOR_SIZE_SHIFT_BIT;
    sptdwb.sptd.TimeOutValue = timeoutVal;
